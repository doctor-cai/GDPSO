...
figure(3)
% subplot(1,2,1);
data = [0.0033976,0.763274,0.455667,0.090418,10,10,10;%一行表示一个数据所有算法得到的结果
    0.02878,1.0355,1.292639,0.5029,10,10,10;
    0.0368446,1.368761,36.552046,6.970292,10,10,10;
    0.23561,2.282507,109.958445,1.933909,10,10,10];
b = bar(data,0.4);
ch = get(b,'children');
% axis xy;
...
legend('DPSO','GA-net','Meme-net','GN','MOGA-net','MOCD','MOEA/D-net');
...
figure(5)
% subplot(1,2,2);
time = [0.001185 0.00275945 0.00611788 0.0122654 20.2631];
node=[34 62 105 118 1589];
edge=[78 159 613 200 2742];
mn=[node.^edge];
y1 = plot(node,time);
